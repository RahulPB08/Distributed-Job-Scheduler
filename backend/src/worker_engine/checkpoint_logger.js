...
export class CheckpointLogger {
  ...
  static heartbeat(workerId, activeJobs, concurrency, cpu, memMb) {
    const time = this.timestamp();
    process.stdout.write(`${COLORS.dim}[${time}]${COLORS.reset} ${COLORS.magenta}♥ [HEARTBEAT]${COLORS.reset} Worker: ${COLORS.bright}${workerId}${COLORS.reset} | Active: ${activeJobs}/${concurrency} | CPU: ${cpu}% | RAM: ${memMb}MB\n`);
