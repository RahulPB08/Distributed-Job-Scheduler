...
export class HeartbeatReporter {
  ...
  async sendHeartbeat() {
    ...
    try {
      ...
      // Check if worker was marked stopped or removed from database
      const workerStatus = await new Promise((resolve) => {
        this.db.get("SELECT status FROM workers WHERE id = ?", [this.workerId], (err, row) => resolve(row));
      });
      if (!workerStatus || workerStatus.status === 'stopped') {
        this.stop();
        return;
      }
