...
export class DlqHandler {
   ...
   * @returns {Promise<object|null>} Structured diagnostic JSON matching the schema, or null on failure
   */
  static async generateAIDiagnosticSummary(error, job) {
    ...
    if (!apiKey) {
      console.error('[AI_DIAGNOSTICS] Skipped: GEMINI_API_KEY is not configured in environment.');
      return null;
    }
    ...
    if (timeSinceLastCall < DlqHandler.THROTTLE_INTERVAL_MS) {
      console.warn(`[AI_DIAGNOSTICS] Throttled: Request rejected to protect rate limits (${Math.ceil((DlqHandler.THROTTLE_INTERVAL_MS - timeSinceLastCall) / 1000)}s cooldown remaining).`);
      return null;
    }
...
${JSON.stringify(errorDetails, null, 2)}`;
      ...
      // Graceful error handling: log and return null without throwing or crashing
      console.error('[AI_DIAGNOSTICS] Gemini API diagnostic failed:', apiError?.message || apiError);
      return null;
    }
  }
