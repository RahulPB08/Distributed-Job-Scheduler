...
export class WorkerInstance {
  ...
  async pollAndExecute() {
    ...
    try {
      ...
      // ─────────────────────────────────────────────────────────────────────────────
      // QUEUE SHARD ROUTING & LOAD BALANCING:
      // Each queue operates as an independent partition / shard. Active shards are
      // evaluated in priority order (Shard Priority DESC). Workers round-robin or
      // prioritize high-value shards, preventing starvation of critical jobs.
      // ─────────────────────────────────────────────────────────────────────────────
      const activeQueues = queues.filter((q) => !q.is_paused);
      activeQueues.sort((a, b) => (b.priority || 10) - (a.priority || 10));
   ...
   * Discovers the set of queue shards accessible to this worker node.
   * - If `--assigned-queues` is specified (e.g. `shard-1,high-priority`), the worker
   *   is pinned strictly to consume only those assigned queue partitions.
   * - Otherwise, the worker consumes dynamically across all unpaused project shards.
   */
  async discoverQueues() {
    return new Promise((resolve, reject) => {
      ...
      if (this.assignedQueues.length > 0) {
        const placeholders = this.assignedQueues.map(() => '?').join(',');
        sql += ` AND (q.name IN (${placeholders}) OR q.id IN (${placeholders}))`;
        params.push(...this.assignedQueues, ...this.assignedQueues);
      }
